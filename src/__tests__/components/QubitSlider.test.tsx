...
describe("QubitSlider", () => {
  it("renders the heading", () => {
    renderWithDict(<QubitSlider value={1000} onChange={vi.fn()} />);
    expect(screen.getByText("Qubit Count")).toBeInTheDocument();
  });

  it("displays the current qubit value", () => {
    renderWithDict(<QubitSlider value={5000} onChange={vi.fn()} />);
    expect(screen.getByText("5,000")).toBeInTheDocument();
  });

  it("renders the range slider with correct attributes", () => {
    renderWithDict(<QubitSlider value={2000} onChange={vi.fn()} />);
  ...
  it("calls onChange when slider value changes", () => {
    ...
    renderWithDict(<QubitSlider value={1000} onChange={handleChange} />);
    const slider = screen.getByRole("slider");
  ...
  it("shows 0/5 broken when qubit count is low", () => {
    renderWithDict(<QubitSlider value={100} onChange={vi.fn()} />);
    expect(screen.getByText("RSA KEY VULNERABILITY (0/5 broken)")).toBeInTheDocument();
  });

  it("shows ML-KEM as always safe", () => {
    renderWithDict(<QubitSlider value={10000} onChange={vi.fn()} />);
